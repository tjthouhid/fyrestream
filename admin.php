    ...
    case 'market':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'orders':
          ...
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query("SELECT COUNT(*) as count FROM orders INNER JOIN users u1 ON orders.seller_id = u1.user_id INNER JOIN users u2 ON orders.buyer_id = u2.user_id") or _error('SQL_ERROR');
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
          $params['url'] = $system['system_url'] . '/' . $control_panel['url'] . '/market/orders?page=%s';
          $pager = new Pager($params);
          $limit_query = $pager->getLimitSql();
          $get_rows = $db->query("SELECT orders.*, u1.user_name AS buyer_username, u1.user_firstname AS buyer_firstname, u1.user_lastname AS buyer_lastname, u1.user_picture AS buyer_user_picture, u1.user_gender AS buyer_user_gender, u2.user_name AS seller_username, u2.user_firstname AS seller_firstname, u2.user_lastname AS seller_lastname, u2.user_picture AS seller_user_picture, u2.user_gender AS seller_user_gender FROM orders INNER JOIN users u1 ON orders.seller_id = u1.user_id INNER JOIN users u2 ON orders.buyer_id = u2.user_id ORDER BY orders.order_id DESC " . $limit_query) or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
        ...
        case 'find_orders':
          ...
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query(sprintf('SELECT COUNT(*) as count FROM orders INNER JOIN users u1 ON orders.seller_id = u1.user_id INNER JOIN users u2 ON orders.buyer_id = u2.user_id WHERE orders.order_hash LIKE %1$s', secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
          $params['url'] = $system['system_url'] . '/' . $control_panel['url'] . '/market/find_orders?query=' . $_GET['query'] . '&page=%s';
          $pager = new Pager($params);
          $limit_query = $pager->getLimitSql();
          $get_rows = $db->query(sprintf('SELECT orders.*, u1.user_name AS buyer_username, u1.user_firstname AS buyer_firstname, u1.user_lastname AS buyer_lastname, u1.user_picture AS buyer_user_picture, u1.user_gender AS buyer_user_gender, u2.user_name AS seller_username, u2.user_firstname AS seller_firstname, u2.user_lastname AS seller_lastname, u2.user_picture AS seller_user_picture, u2.user_gender AS seller_user_gender FROM orders INNER JOIN users u1 ON orders.seller_id = u1.user_id INNER JOIN users u2 ON orders.buyer_id = u2.user_id WHERE orders.order_hash LIKE %1$s ' . $limit_query, secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              /* prepare buyer's info */
              $row['buyer_fullname'] = ($system['show_usernames_enabled']) ? $row['buyer_username'] : $row['buyer_firstname'] . ' ' . $row['buyer_lastname'];
