...
$(function () {
  ...
  $('body').on('click', '#js_live-start', async function () {
    ...
    try {
      ...
      /* update the server side */
      /* get live tips value */
      $.post(api['live/reaction'], { 'do': 'start', 'agora_channel_name': agora_channel_name, 'video_thumbnail': video_thumbnail, 'node': _this.data('node'), 'node_id': _this.data('node-id'), 'tips_enabled': $('#tips_enabled').is(":checked") }, function (response) {
        /* update live_post_id */
