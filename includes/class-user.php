...
class User
{
    ...
    if (isset($user_id) && isset($user_token)) {
      ...
      if ($get_user->num_rows > 0) {
        ...
        if ($system['funding_enabled']) {
          $this->_data['can_raise_funding'] = $this->check_module_permission('fundings_permission');
        }
  ...
  public function get_friends_ids($user_id = null)
  {
    ...
    $get_friends = $db->query(sprintf(sprintf('
          ...
          FROM (
              ...
              FROM friends AS f
              WHERE f.status = %1$s AND f.user_one_id = %1$s
              UNION
              SELECT
  ...
  public function get_new_people($offset = 0, $random = false)
  {
    ...
    $where .= sprintf("WHERE users.user_id != '1' AND users.user_activated = '1' AND users.user_banned = '0' AND users.user_id != %s AND users.user_id NOT IN (%s)", secure($this->_data['user_id'], 'int'), $this->spread_ids($old_people_ids));
  ...
  public function get_stories()
  {
    ...
    if ($system['newsfeed_source'] == "default") {
      $source_query = sprintf(" AND ( is_ads = '1' OR stories.user_id = %s OR stories.user_id IN (%s)) ", $this->_data['user_id'], $this->spread_ids($this->_data['friends_ids']));
    } else {
  ...
  public function scraper($url)
  {
    ...
    if ($info) {
      ...
      if ($return['source_type'] === 'rich' && preg_match('/wp-embedded-content/', $info->code->html)) {
  ...
  public function edit_product($post_id, $message, $args = [])
  {
    ...
    /* update product */
    $db->query(sprintf("UPDATE posts_products SET name = %s, price = %s, currency_id = %s, quantity = %s, category_id = %s, status = %s, location = %s WHERE post_id = %s", secure($args['name']), secure($args['price'], 'float'), secure($args['currency'], 'int'), secure($args['quantity'], 'int'), secure($args['category'], 'int'), secure($args['status']), secure($args['location']), secure($post_id, 'int'))) or _error('SQL_ERROR_THROWEN');
  }
   ...
   * @return integer
   */
  public function post_article($publish_to, $page_id, $group_id, $event_id, $title, $text, $cover, $category_id, $tags, $tips_enabled)
  {
    global $db, $system, $date;
    ...
    $check_category = $db->query(sprintf("SELECT COUNT(*) as count FROM blogs_categories WHERE category_id = %s", secure($category_id, 'int'))) or _error('SQL_ERROR_THROWEN');
    if ($check_category->fetch_assoc()['count'] == 0) {
      throw new Exception(__("You must select valid category for your article"));
    }
    /* prepare text */
    ...
    $tips_enabled = ($publish_to != 'page' && $this->_data['can_receive_tip'] && $tips_enabled) ? '1' : '0';
    /* publish to */
    switch ($publish_to) {
      case 'timeline':
        /* insert the post */
        $db->query(sprintf("INSERT INTO posts (user_id, user_type, post_type, tips_enabled, time, privacy) VALUES (%s, 'user', 'article', %s, %s, 'public')", secure($this->_data['user_id'], 'int'), secure($tips_enabled), secure($date))) or _error('SQL_ERROR_THROWEN');
        $post_id = $db->insert_id;
        break;
      ...
      case 'page':
        ...
        /* insert the post */
        $db->query(sprintf("INSERT INTO posts (user_id, user_type, post_type, tips_enabled, time, privacy) VALUES (%s, 'page', 'article', %s, %s, 'public')", secure($page_id, 'int'), secure($tips_enabled), secure($date))) or _error('SQL_ERROR_THROWEN');
        $post_id = $db->insert_id;
        break;
      ...
      case 'group':
        ...
        /* insert the post */
        $group_approved = ($group['group_publish_approval_enabled'] && !$group['i_admin']) ? "0" : "1";
        $db->query(sprintf("INSERT INTO posts (user_id, user_type, post_type, tips_enabled, time, privacy, in_group, group_id, group_approved) VALUES (%s, 'user', 'article', %s, %s, 'custom', '1', %s, %s)", secure($this->_data['user_id'], 'int'), secure($tips_enabled), secure($date), secure($group_id, 'int'), secure($group_approved))) or _error('SQL_ERROR_THROWEN');
        $post_id = $db->insert_id;
        /* post in_group notification */
        if (!$group_approved) {
          /* send notification to group admin */
      ...
      case 'event':
        ...
        /* insert the post */
        $event_approved = ($event['event_publish_approval_enabled'] && !$event['i_admin']) ? "0" : "1";
        $db->query(sprintf("INSERT INTO posts (user_id, user_type, post_type, tips_enabled, time, privacy, in_event, event_id, event_approved) VALUES (%s, 'user', 'article', %s, %s, 'custom', '1', %s, %s)", secure($this->_data['user_id'], 'int'), secure($tips_enabled), secure($date), secure($event_id, 'int'), secure($event_approved))) or _error('SQL_ERROR_THROWEN');
        $post_id = $db->insert_id;
        /* post in_event notification */
        if (!$event_approved) {
          /* send notification to event admin */
